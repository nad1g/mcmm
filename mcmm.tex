...
% where f(x) is a pdf (i.e., f(x) >= 0 for all x and integral (f(x) dx) = 1)
% 
% If g(x) = x, we obtain the expected value; If g(x) = x^2, we obtain the 
...
% 
% *Key idea: MC method says:
% 
% integral ( g(x) f(x) dx ) approx = 1/N sum( g(x'_1) , g(x'_2) , ... , g(x'_N) )
% where x'_1, x'_2,... are "sampled" from the pdf f(x).
% 
% **** Example ****
